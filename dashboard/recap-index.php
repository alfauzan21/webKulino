...
<body>
  ...
  `;
    ...
    // ==================== DEBUG ACTIVITY DATA ====================
    function debugActivityData(data) {
      console.group('📋 Activity Data Debug');
      console.log('Total activities:', data.activity?.length || 0);

      if (data.activity && data.activity.length > 0) {
        console.log('Sample activity:', data.activity[0]);
        console.table(data.activity.slice(0, 5).map(act => ({
          Time: act.time,
          IP: act.ip,
          Country: act.country,
          City: act.city,
          Street: act.street,
          HouseNo: act.house_number,
          District: act.district,
          Subdistrict: act.subdistrict,
          PostalCode: act.postal_code,
          HasGPS: !!(act.latitude && act.longitude)
        })));
      }
